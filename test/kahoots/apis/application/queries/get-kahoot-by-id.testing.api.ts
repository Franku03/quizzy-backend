...
export class GetKahootByIdTestAPI {
    ...
    private configureDefaultMocks(): void {
        this.mediaMock.enrichKahoot.mockImplementation(async (snapshot: KahootSnapshot) => snapshot);
    ...
    private getSnapshotOrThrow(): KahootSnapshot {
        if (!this.existingSnapshot) throw new Error("Falta: givenAnExistingKahootSnapshot()");
