...
import { KahootSecurityMother } from 'test/kahoots/object-mothers/application/authorization-strategies/kahoowOwnership.strategy.mother';
...
export class KahootSecurityTestAPI {
   ...
   */
  public async whenTheUserTriesToAccessAs(
    userId: string,
    accion: string,
  ): Promise<this> {
    ...
    const context = {
      findKahootByIdEither: () =>
        Promise.resolve(Either.makeRight(this.resource)),
    };

    // Ejecución de la lógica de autorización
    // Ahora usamos KahootFetcher directamente para evitar errores de unsafe-argument y explicit-any
    this.result = await this.sut.authorize(
      ...
      context as unknown as KahootFetcher,
    );
    return this;
  }
