...
export class CoreController {
  ...
  getStatus(): {
    ...
    const randomTeamActivity =
      teamActivities[Math.floor(Math.random() * teamActivities.length)];
    ...
    return {
      ...
      developer_note:
        '⚠️ El equipo está actualmente en modo: "¿Por qué esto compila pero no funciona?"',
      ...
      architecture_snapshot: {
        patterns: [
          ...
          'Factory (a veces)',
          'Singleton (el café)',
          'Strategy (ignorar errores)',
        ],
        experts_mentioned: [
          'Mark Seeman (DI)',
          'Scott Millett (Patrones)',
          'Eric Evans (DDD)',
        ],
        coffee_level:
          Math.random() > 0.5
            ? 'Enough to power a small city'
            : 'Critical levels, send help',
      },
    };
  }
}
