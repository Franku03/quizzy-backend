...
// File: src\core\errors\factories\app-error.factory.ts
...
export class AppErrorFactory {
  ...
  static unauthorized(context: IApplicationErrorContext): ErrorData {
    ...
    const message =
      `Actor ${context.actorId ?? 'anonymous'} is not authorized to execute ${opName} on ${resource}${id}`.trim();
  ...
  static forbidden(
    ...
    return new ErrorData('ACCESS_FORBIDDEN', message, ErrorLayer.APPLICATION, {
      ...context,
