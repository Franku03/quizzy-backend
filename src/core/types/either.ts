...
export class Either<TLeft, TRight> {
  ...
  mapLeft<TNewLeft>(fn: (err: TLeft) => TNewLeft): Either<TNewLeft, TRight> {
    if (this.isRight())
      return Either.makeRight<TNewLeft, TRight>(this.getRight());
  ...
  async tapChainAsync(
    fn: (val: TRight) => Promise<Either<TLeft, unknown>>,
  ): Promise<Either<TLeft, TRight>> {
    if (this.isLeft()) return Either.makeLeft<TLeft, TRight>(this.getLeft());
    const result = await fn(this.getRight());
    if (result.isLeft())
      return Either.makeLeft<TLeft, TRight>(result.getLeft());
