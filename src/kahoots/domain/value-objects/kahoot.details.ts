...
export class KahootDetails extends ValueObject<KahootDetailsProps> {
  ...
  public isValidDetails(): Either<ErrorData, boolean> {
    ...
    if (
      !this.properties.title.hasValue() ||
      !this.properties.category.hasValue()
    ) {
      return Either.makeLeft(
        DomainErrorFactory.validation(
          context,
  ...
  public get category(): Optional<KahootCategoryEnum> {
    return this.properties.category;
  }

  public getSnapshot(): KahootDetailsSnapshot {
    return {
      title: this.properties.title.hasValue()
        ? this.properties.title.getValue()
        : undefined,
      description: this.properties.description.hasValue()
        ? this.properties.description.getValue()
        : undefined,
      category: this.properties.category.hasValue()
        ? this.properties.category.getValue()
        : undefined,
    };
  }
}
