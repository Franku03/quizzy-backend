...
// File: src\kahoots\domain\factories\kahoot.factory.ts
...
interface SlideCreator {
  create(props: SlideProps, id: SlideId): Either<ErrorData, Slide>;
}
...
export class KahootFactory {
  private static readonly SlideCreatorsMap: Record<
    SlideTypeEnum,
    SlideCreator
  > = {
    [SlideTypeEnum.SINGLE]: SingleChoiceSlide as unknown as SlideCreator,
    [SlideTypeEnum.MULTIPLE]: MultipleChoiceSlide as unknown as SlideCreator,
    [SlideTypeEnum.TRUE_FALSE]: TrueFalseSlide as unknown as SlideCreator,
    [SlideTypeEnum.SHORT_ANSWER]: ShortAnswerSlide as unknown as SlideCreator,
    [SlideTypeEnum.SLIDE]: DisplaySlide as unknown as SlideCreator,
  };

  // --- Helpers de Construcción Interna ---

  public static buildOption(input: OptionInput): Either<ErrorData, Option> {
    return this.wrapOptional(input.optionImage, (v) => ImageId.create(v)).chain(
      (imgIdVO) => Option.create(input.text, input.isCorrect, imgIdVO),
    );
  }

  private static buildOptionFromSnapshot(
    snap: OptionSnapshot,
  ...
  public static buildSlideFromInput(
    input: SlideInput,
    pos: number,
  ): Either<ErrorData, Slide> {
    ...
    const descriptionToProcess =
      (input.slideType as SlideTypeEnum) === SlideTypeEnum.SLIDE
        ? input.description
        : undefined;

    return SlideId.create(input.id).chain((sId) =>
      this.assembleSlideProps(
        pos,
        input.timeLimit,
        input.points,
        input.question,
        ...
        this.resolveSlideCreator(input.slideType as SlideTypeEnum).chain(
          (Creator) => Creator.create(props, sId),
        ),
      ),
    );
  }

  private static buildSlideFromSnapshot(
    snap: SlideSnapshot,
    pos: number,
  ): Either<ErrorData, Slide> {
    const options: Option[] = [];
    ...
    const descriptionToProcess =
      snap.slideType === SlideTypeEnum.SLIDE ? snap.descriptionText : undefined;
    ...
    return SlideId.create(snap.id).chain((sId) =>
      this.assembleSlideProps(
        ...
        this.resolveSlideCreator(snap.slideType).chain((Creator) =>
          Creator.create(props, sId),
        ),
      ),
    );
  }
  ...
  public static createFromInput(input: KahootInput): Either<ErrorData, Kahoot> {
    return this.processSlides(input.slides, (s, p) =>
      this.buildSlideFromInput(s, p),
    ).chain((slidesMap) =>
  ...
  public static reconstructFromSnapshot(
    snapshot: KahootSnapshot,
  ): Either<ErrorData, Kahoot> {
    return this.processSlides(snapshot.slides, (s, p) =>
      this.buildSlideFromSnapshot(s, p),
    ).chain((slidesMap) =>
      this.assembleStyling(
        snapshot.styling.themeId,
        snapshot.styling.imageId,
      ).chain((styling) =>
        this.assembleDetails(
          ...
          KahootId.create(snapshot.id).chain((kId) =>
            UserId.create(snapshot.authorId).chain((authorId) =>
              KahootStatus.create(snapshot.status).chain((status) =>
                VisibilityStatus.create(snapshot.visibility).chain(
                  (visibility) =>
  ...
  private static assembleSlideProps(
    ...
    return TimeLimitSeconds.create(time).chain((timeVO) =>
      this.wrapOptional(pts, (v) => Points.create(v)).chain((pointsVO) =>
        this.wrapOptional(ques, (v) => Question.create(v)).chain((quesVO) =>
          this.wrapOptional(img, (v) => ImageId.create(v)).chain((imgVO) =>
            this.wrapOptional(desc?.trim() ? desc : undefined, (v) =>
              Description.create(v),
            ).map(
              (descVO) =>
                ({
                  position: pos,
  ...
  public static assembleStyling(
    themeId: string,
    imageId?: string,
  ): Either<ErrorData, KahootStyling> {
    return ThemeId.create(themeId).chain((tId) => {
      const cleanImageId = imageId?.trim() ? imageId : undefined;
      return this.wrapOptional(cleanImageId, (v) => ImageId.create(v)).chain(
        (imgVO) => KahootStyling.create(imgVO, tId),
      );
    });
  }

  private static resolveSlideCreator(
    type: SlideTypeEnum,
  ): Either<ErrorData, SlideCreator> {
