      ...
      if (pointValue !== 0) {
        return Either.makeLeft(
          DomainErrorFactory.validation(
            context,
            { points: ['MUST_BE_ZERO'] },
            'Display slides must have 0 points.',
          ),
        );
      }
    }
      ...
      if (optionsOptional.getValue().length > 0) {
        return Either.makeLeft(
          DomainErrorFactory.validation(
            context,
            { options: ['NOT_ALLOWED'] },
            'Display slides cannot have options.',
          ),
        );
      }
    }
    ...
    if (!this.properties.question.hasValue()) {
      return Either.makeLeft(
        DomainErrorFactory.validation(
          context,
          { question: ['REQUIRED'] },
          'Display slide must have a title.',
        ),
      );
    }
    ...
    if (!this.properties.description.hasValue()) {
      return Either.makeLeft(
        DomainErrorFactory.validation(
          ...
          'Display slide must have a description.',
        ),
      );
    }
