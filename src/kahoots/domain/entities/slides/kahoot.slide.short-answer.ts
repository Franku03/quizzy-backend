...
// File: src\kahoots\domain\entities\slides\kahoot.slide.short-answer.ts
...
export class ShortAnswerSlide extends Slide {
  ...
  public static create(
    props: SlideProps,
    id: SlideId,
  ): Either<ErrorData, ShortAnswerSlide> {
    ...
    if (!pointsOptional.hasValue()) {
      return Either.makeLeft(
        DomainErrorFactory.validation(
          context,
          { points: ['REQUIRED'] },
          'Points are mandatory for Short Answer slides.',
        ),
      );
    }
    ...
    if (this.properties.description.hasValue()) {
      return Either.makeLeft(
        DomainErrorFactory.validation(
          ...
          'Short Answer slides do not support descriptions.',
        ),
      );
    }
    ...
    if (optionsOptional.hasValue()) {
      ...
      if (optionsArray.length > maxOption) {
        return Either.makeLeft(
          DomainErrorFactory.validation(
            ...
            `Short Answer slides cannot exceed ${maxOption} correct answers.`,
          ),
        );
      }

      const invalidOption = optionsArray.find(
        (o) =>
        ...
        if (!invalidOption.isWithinLengthLimit(MAX_OPTION_CHARS_TYPEANSWER)) {
          return Either.makeLeft(
            DomainErrorFactory.validation(
              context,
              { options: ['TOO_LONG'] },
              `A short answer cannot exceed ${MAX_OPTION_CHARS_TYPEANSWER} characters.`,
            ),
          );
        }
        return Either.makeLeft(
          DomainErrorFactory.validation(
            ...
            'Each answer must be text-only and cannot contain an image.',
          ),
        );
      }

      const incorrectOptionsCount = optionsArray.filter(
        (o) => !o.isCorrect,
      ).length;
      if (incorrectOptionsCount > 0) {
        return Either.makeLeft(
          DomainErrorFactory.validation(
            context,
            { options: ['MUST_BE_CORRECT'] },
            'All options in a Short Answer slide must be marked as correct.',
          ),
        );
      }
    }
    ...
    if (!this.properties.question.hasValue()) {
      return Either.makeLeft(
        DomainErrorFactory.validation(
          context,
          { question: ['REQUIRED'] },
          'Short Answer slide must have a title.',
        ),
      );
    }
    ...
    if (optionsArray.length < 1) {
      return Either.makeLeft(
        DomainErrorFactory.validation(
          ...
          `Short Answer slide must have between 1 and ${this.getMaxOptions()} correct answers.`,
        ),
      );
    }
