...
// File: src\kahoots\domain\helpers\test-knowledge.strategy.ts
...
export class TestKnowledgeEvaluationStrategy implements EvaluationStrategy {
  public evaluateAnswer(submission: Submission, options: Option[]): Result {
    ...
    if (!answerIndexOptional.hasValue()) {
      ...
      return new Result(
        submission,
        new Optional<Score>(Score.create(0)),
        false,
      );
    }
    ...
    if (questionTimer <= 0) {
      ...
      return new Result(
        submission,
        new Optional(Score.create(finalScore)),
        isCorrect,
      );
    }
