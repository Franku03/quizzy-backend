...
} from '../value-objects/kahoot.visibility-status';
...
export class Kahoot extends AggregateRoot<KahootProps, KahootId> {
  ...
  public static create(
    ...
    // Eliminamos el chequeo de props.author, props.status, etc.,
    // porque el linter detecta que, según KahootProps, estos valores
    // nunca pueden ser nulos o indefinidos.
  ...
  protected checkInvariants(): Either<ErrorData, void> {
    ...
    if (
      this.properties.status.value === KahootStatusEnum.DRAFT &&
  ...
  private checkPublishingReadiness(): Either<ErrorData, void> {
    ...
    if (this.properties.details.hasValue()) {
      ...
      detailsResult = Either.makeLeft(
        DomainErrorFactory.validation(
          context,
          { details: ['MISSING'] },
          'Details are required for publishing.',
        ),
      );
    }
  ...
  public changeStatus(newStatus: string): Either<ErrorData, void> {
    ...
    switch (newStatus as KahootStatusEnum) {
  ...
  public makePublic(): Either<ErrorData, void> {
    const visibilityResult = VisibilityStatus.create(
      VisibilityStatusEnum.PUBLIC,
    );
    if (visibilityResult.isLeft())
      return Either.makeLeft(visibilityResult.getLeft());
  ...
  public changeVisibility(newVisibility: string): Either<ErrorData, void> {
    ...
    switch (newVisibility as VisibilityStatusEnum) {
  ...
  public updateStyling(
    newStyling: KahootStyling,
  ): Either<ErrorData, undefined> {
    this.properties.styling = newStyling;
    return Either.makeRight<ErrorData, undefined>(undefined);
  }
  ...
  public removeSlideOptionByIndex(
    id: SlideId,
    idx: number,
  ): Either<ErrorData, void> {
  ...
  public evaluateAnswer(submission: Submission): Result {
    ...
    if (!slide)
      throw DomainErrorFactory.validation(
        context,
        { slideId: ['NOT_FOUND'] },
        'Slide not found.',
      );
    if (this.isDraft())
      throw DomainErrorFactory.validation(
        context,
        { status: ['IS_DRAFT'] },
        'Evaluation prohibited in DRAFT mode.',
      );
  ...
  public getNextSlideSnapshotById(
    currentId: SlideId | null,
  ): SlideSnapshot | null {
    const sorted = this.getSortedSlides();
    if (sorted.length === 0) return null;
    const index = currentId
      ? sorted.findIndex((s) => s.id.equals(currentId))
      : -1;
    return sorted[index + 1]?.getSnapshot() ?? null;
  }

  public getNextSlideSnapshotByIndex(currentIndex = -1): SlideSnapshot | null {
    if (this.properties.slides.size === 0) return null;

    const sortedSlides = this.getSortedSlides();
    const nextIndex = currentIndex === -1 ? 0 : currentIndex + 1;

    return sortedSlides[nextIndex]?.getSnapshot() ?? null;
  }
  ...
  public get authorId(): string {
    return this.properties.author.value;
  }
  public get createdAt(): DateISO {
    return this.properties.createdAt;
  }
  public get status(): KahootStatus {
    return this.properties.status;
  }
  public get visibility(): VisibilityStatus {
    return this.properties.visibility;
  }
  public get styling(): KahootStyling {
    return this.properties.styling;
  }
  public get details(): Optional<KahootDetails> {
    return this.properties.details;
  }
  public get slides(): Map<string, Slide> {
    return this.properties.slides;
  }
  public get playCount(): PlayNumber {
    return this.properties.playCount;
  }
