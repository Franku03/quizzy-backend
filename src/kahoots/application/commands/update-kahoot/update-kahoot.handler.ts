...
import {
  KahootFactory,
  SlideInput,
} from 'src/kahoots/domain/factories/kahoot.factory';
...
export class UpdateKahootHandler implements ICommandHandler<UpdateKahootCommand> {
  constructor(
    ...
    private readonly kahootMapper: IMapper<
      KahootSnapshot,
  ...
  async execute(
    ...
    const appContext = createKahootAppContext(
      'updateKahoot',
      command.kahootId,
      command.userId,
    );
    ...
    return pipeAsync<ErrorData, KahootHandlerResponseDto>(
      ...
      (k) =>
        k
          .chain((kahoot: Kahoot) => this.applyUpdates(kahoot, command))
          //Agregando contexto de app extra a los posibles errores de dominio
          .mapLeft((err) => err.setContext(appContext)),

      // 3. Persistencia
      (k) =>
        k.tapChainAsync((kahoot: Kahoot) =>
          this.kahootRepository.saveKahootEither(kahoot),
        ),

      // 4. Efectos secundarios (Cleanup)
      (k) => k.tapChainAsync((kahoot: Kahoot) => this.runSideEffects(kahoot)),

      // 5. Transformación Final (Snapshot -> Enriquecer -> DTO)
      (k) => k.map((kahoot: Kahoot) => kahoot.getSnapshot()),
      (k) =>
        k.mapAsync((snapshot: KahootSnapshot) =>
          this.mediaService.enrichKahoot(snapshot),
        ),
      (k) =>
        k.map((enriched: KahootSnapshot) => this.kahootMapper.map(enriched)),
    );
  }

  private applyUpdates(
    kahoot: Kahoot,
    command: UpdateKahootCommand,
    ...
    return this.processSlidesMap(command.slides ?? [])
      ...
      .chain(() =>
        KahootFactory.assembleStyling(command.themeId, command.imageId),
      )
  ...
  private processSlidesMap(
    rawSlides: KahootSlideCommand[],
  ): Either<ErrorData, Map<string, Slide>> {
    ...
    return KahootFactory.processSlides(
      slidesInput,
      (s: SlideInput, p: number) => KahootFactory.buildSlideFromInput(s, p),
    );
  }

  private runSideEffects(kahoot: Kahoot): Promise<Either<ErrorData, Kahoot>> {
    // Se lanza la limpieza pero NO se usa 'await', así que corre en segundo plano
    this.attemptCleanup
      .cleanupById(new KahootId(kahoot.id.value))
      .catch((err: unknown) => {
        this.logger.error(
          `Error cleaning up attempts for ${kahoot.id.value}`,
          err instanceof Error ? err : new Error(String(err)),
        );
      });

    return Promise.resolve(Either.makeRight(kahoot));
  }
}
