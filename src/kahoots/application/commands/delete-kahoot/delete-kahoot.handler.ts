...
export class DeleteKahootHandler implements ICommandHandler<DeleteKahootCommand> {
  ...
  async execute(
  ...
  ): Promise<Either<ErrorData, undefined>> {
    ...
    // 1. Usamos 'undefined' en lugar de 'void' para el tipo genérico de Either
    return pipeAsync<ErrorData, undefined>(
      Either.makeRight<ErrorData, Kahoot>(
        command.validatedResource as Kahoot,
      ).mapLeft((err) => err.setContext(appContext)),

      (k) =>
        k.chainAsync((kahoot: Kahoot) =>
          this.kahootRepository.deleteKahootEither(kahoot.id.value),
        ),

      // 2. Se eliminó la aserción innecesaria y el non-null assertion (!)
      // usando un fallback seguro si fuera necesario o confiando en el tipo de 'command'
      (res: Either<ErrorData, undefined>) =>
        this.handlePostDeleteEffects(res, command.kahootId),
    );
  }
  ...
  private handlePostDeleteEffects(
    result: Either<ErrorData, undefined>,
    id: string,
  ): Either<ErrorData, undefined> {
    if (result.isRight() && id) {
      this.attemptCleanup
        .cleanupById(new KahootId(id))
        .catch((err: unknown) => {
          this.logger.error(`Cleanup failed for kahoot ${id}`, err);
        });
    }
    return result;
  }
