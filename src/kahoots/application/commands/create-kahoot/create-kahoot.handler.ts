...
// File: src\kahoots\application\commands\create-kahoot\create-kahoot.handler.ts
...
import {
  KahootFactory,
  SlideInput,
} from '../../../domain/factories/kahoot.factory';
...
export class CreateKahootHandler implements ICommandHandler<CreateKahootCommand> {
  ...
  async execute(
    ...
    return pipeAsync<ErrorData, KahootHandlerResponseDto>(
      ...
      (k) =>
        k.map((enrichedSnapshot: KahootSnapshot) =>
          this.kahootMapper.map(enrichedSnapshot),
        ),
    );
  }
