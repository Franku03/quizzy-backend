...
// File: src\users\application\commands\create-user\create-user.handler.ts
...
export class CreateUserHandler implements ICommandHandler<CreateUserCommand> {
  ...
  async execute(command: CreateUserCommand): Promise<Either<ErrorData, string>> {
    ...
    if (emailExists) {
      return Either.makeLeft(
        new ErrorData(
          'CONFLICT',
          CREATE_USER_ERROR_CODES.USER_EMAIL_ALREADY_EXISTS,
          ErrorLayer.DOMAIN,
          { isEmail: true }
        )
      );
    ...
    if (usernameExists) {
        return Either.makeLeft(
          new ErrorData(
            'CONFLICT',
            CREATE_USER_ERROR_CODES.USER_USERNAME_ALREADY_EXISTS,
            ErrorLayer.DOMAIN,
            { isUsername: true }
          )
        );
