...
import type { ISoloAttemptQueryDao } from '../../queries/ports/attempts.dao.port';
...
export class SubmitAnswerHandler implements ICommandHandler<SubmitAnswerCommand> {
  ...
  async execute(command: SubmitAnswerCommand): Promise<any> {
    ...
    if (!attemptOptional.hasValue()) {
      throw new Error(`${ATTEMPT_ERROR_CODES.ATTEMPT_NOT_FOUND}: Attempt ${attemptId.value} not found`);
    }
    ...
    if (!attempt.isInProgress()) {
      throw new Error(
        `${ATTEMPT_ERROR_CODES.ATTEMPT_NOT_IN_PROGRESS}: Attempt ${attemptId.value} is not in progress`
      );
    }
