...
export class CloudinaryErrorMapper implements IErrorMapper<
  unknown,
  IExternalServiceErrorContext
> {
  public toErrorData(
    error: unknown,
    context: IExternalServiceErrorContext,
  ): ErrorData {
    const err = error as CloudinaryRawError;
    ...
    if (httpCode === 429 || httpCode === 413) {
      const message =
        httpCode === 429
          ? 'Rate limit exceeded in Cloudinary.'
          : 'File too large (413).';
      return new ErrorData(
    ...
    if (
      ...
      return new ErrorData(
        ...
        `Resource not found: ${context.resourceId ?? 'unknown'}.`,
        ErrorLayer.EXTERNAL,
        baseDetails,
        safeError,
      );
  ...
  private buildValidationMessage(
    error: CloudinaryRawError,
    context: IExternalServiceErrorContext,
  ): string {
    const message = error.message ?? 'Invalid request to Cloudinary';
    if (message.includes('already exists'))
      return `File already exists: ${context.resourceId ?? 'unknown'}`;
