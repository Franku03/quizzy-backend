...
// File: src\media\application\queries\get-theme-by-id\get-theme-by-id.handler.ts
...
export class GetThemeByIdHandler implements IQueryHandler<GetThemeByIdQuery> {
  ...
  async execute(
    ...
    return pipeAsync<ErrorData, ThemeResponse>(
      ...
      // Tipamos el result como Either de AssetMetadataRecord (no nulo)
      (result: Either<ErrorData, AssetMetadataRecord>) =>
        result.map((record) => this.mapToEnrichedResponse(record)),
    );
  }
