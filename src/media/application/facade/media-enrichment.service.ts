...
import {
  PaginatedKahootListReadModel,
  KahootListReadModel,
} from 'src/explore/application/read-models/kahoot-list.read-model';
...
import { BackofficeNotificationPaginationReadModel, BackofficeNotificationReadModel } from 'src/backoffice/application/read-model/backoffice-notifications.read.model';
...
export class MediaEnrichmentService {
  ...
  public async enrichKahoot(kahoot: KahootSnapshot): Promise<KahootSnapshot> {
    ...
    if (kahoot.slides.length > 0) {
      const slideHandler =
        this.handlerFactory.createAssetHandler<SlideSnapshot>(urlMap);
      await Promise.all(
  ...
  public async enrichKahootList(
    items: KahootListReadModel[],
  ): Promise<KahootListReadModel[]> {
    ...
    const handler =
      this.handlerFactory.createAssetHandler<KahootListReadModel>(urlMap);

    return Promise.all(items.map((item) => handler.handle(item)));
  }

  public async enrichPaginatedKahootList(
    list: PaginatedKahootListReadModel,
  ): Promise<PaginatedKahootListReadModel> {
    return this.enrich(list);
  }

  public async enrichAttemptReport(
    report: AttemptReportReadModel,
  ): Promise<AttemptReportReadModel> {
    return this.enrich(report);
  }

  public async enrinchLibraryReadModel(
    libraryReadModel: LibraryReadModel,
  ): Promise<LibraryReadModel> {
    return this.enrich(libraryReadModel);
  }

  public async enrichAttemptResume(
    resume: AttemptResumeReadModel,
  ...
  public async enrinchBackofficeNotificationReadModel(
    backofficeUser: BackofficeNotificationReadModel,
  ): Promise<BackofficeNotificationReadModel> {
    return this.enrich(backofficeUser);
  }

  public async enrinchBackofficeNotificationPaginationReadModel(
    backofficeUser: BackofficeNotificationPaginationReadModel,
  ): Promise<BackofficeNotificationPaginationReadModel> {
    return this.enrich(backofficeUser);
  }
  ...
  private async enrichStylingInternal(
    ...
    const themeHandler =
      this.handlerFactory.createThemeHandler<KahootStylingSnapshot>();
    const assetHandler =
      this.handlerFactory.createAssetHandler<KahootStylingSnapshot>(urlMap);
   ...
   */
  private collectAssetIdsFromBatch(items: IHasMediaAssets[]): string[] {
