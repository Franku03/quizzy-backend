// src/database/infrastructure/postgres/modules/media/media.pg-dao.ts
...
export class AssetMetadataDao implements IAssetMetadataDao {
  ...
  /**
   * Mapea la Entity de Postgres al Record de aplicación.
   */
  ...
  async incrementReferenceCount(publicId: string): Promise<Either<ErrorData, void>> {
    ...
    return result.chain((updateRes) => 
      updateRes.affected && updateRes.affected > 0 
  ...
  async decrementReferenceCount(publicId: string): Promise<Either<ErrorData, void>> {
    ...
    // 1. Buscar y validar (mismo comportamiento que Mongo)
  ...
  async findByIds(assetIds: string[]): Promise<Either<ErrorData, AssetMetadataRecord[]>> {
    ...
    return result.map((entities) => entities.map((entity) => this.toRecord(entity)));
  }
  ...
  async findThemes(options?: any): Promise<Either<ErrorData, AssetMetadataRecord[]>> {
    const ctx = this.getCtx('findThemes');
    
    const query = this.repo.createQueryBuilder('asset')
      .where('asset.theme = :theme', { theme: true });

    if (options?.category) query.andWhere('asset.category = :category', { category: options.category });
    if (options?.format) query.andWhere('asset.format = :format', { format: options.format });
    if (options?.mimeType) query.andWhere('asset.mimeType = :mimeType', { mimeType: options.mimeType });
