...
interface PostgresNativeError extends Error {
    ...
    detail?: string;    // Información detallada (e.g., "Key (email)=(test@test.com) already exists.")
    table?: string;
    constraint?: string;
    column?: string;
    dataType?: string;
...
export class PostgresErrorMapper implements IErrorMapper<unknown, IDatabaseErrorContext> {
    ...
    public toErrorData(error: unknown, context: IDatabaseErrorContext): ErrorData {
        ...
        if (pgError.code === '23502') {
            return new ErrorData(
                ...
                `Field ${pgError.column} cannot be null.`,
                ErrorLayer.INFRASTRUCTURE,
                { 
                    ...baseDetails, 
                    postgresCode: '23502', 
        ...
        // 4. ERROR DE SINTAXIS O CONEXIÓN
        // Podrías añadir '08000', '08003', '08006' para problemas de red/conexión
